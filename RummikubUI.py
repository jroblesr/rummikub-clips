...
class RummikubUI(tk.Tk):
    ...
    _letras ={'azul': 'A', 'naranja': 'J', 'negro': 'N', 'rojo': 'R'
              , 'comodin': 'C'}
    # Constantes para las dimensiones de la mesa de juego
    NUM_FILAS_MESA = 10 # Inicialmente 30
    NUM_COLS_MESA = 20 # Inicialmente 25
    ...
    def __init__(self, master=None):
        ...
        self.frame_tablilla_maquina = tk.Frame(self.frame_juego
                                               , name="frame_tablilla_maquina")
        self.frame_tablilla_maquina.configure(background="#097b1b"
                                              , height=100, width=200)
        self.frame_tablilla_maquina.grid(column=0, row=0, sticky="nsew")
        self.tablilla_maquina_botones = self._crear_botones_tablilla(
            self.frame_tablilla_maquina, "maquina", tk.Button)
        ...
        for i in range(self.NUM_FILAS_MESA):
            self.frame_mesa.rowconfigure(i, weight=1)  # Permite que las filas
                                                       # crezcan
            fila = []
            for j in range(self.NUM_COLS_MESA):
                self.frame_mesa.columnconfigure(j, weight=1)  # Permite que las
                                                              # columnas crezcan
                btn_id = f"mesa_{i}_{j}"
                btn = dad.DraggableButton(self.frame_mesa
                                          , name=btn_id, borderwidth=0
                                          , relief=tk.FLAT)
                btn.grid(row=i, column=j, padx=0, pady=0
                         , sticky="nsew")  # sticky para expandir
                fila.append(btn)
        ...
        self.frame_botones.rowconfigure(5, weight=1)  # Permite que la fila
                                                      # del text_area crezca
        self.l_jugando = tk.Label(self.frame_botones, name="l_jugando")
        self.l_jugando.configure(text='HUMANO contra CLIPS'
                                 , relief=tk.SOLID, borderwidth=1)
        self.l_jugando.grid(column=0, row=0, sticky="ew")
        self.l_fichas_en_bolsa = tk.Label(self.frame_botones
                                          , name="l_fichas_en_bolsa")
        self.l_fichas_en_bolsa.configure(text='<...>'
                                         , relief=tk.SOLID, borderwidth=1)
        self.l_fichas_en_bolsa.grid(column=0, row=1, sticky="ew")
        self.bbt_reglas = tk.Button(self.frame_botones
                                    , name="bbt_reglas", text='REGLAS')
        self.bbt_reglas.grid(column=0, row=2, sticky="ew")
        
        # Frame para los botones superiores (NUEVA PARTIDA, 
        # TERMINAR, AYUDA, MOSTRAR)
        self.frame_botones_superiores = tk.Frame(
            self.frame_botones
            , name="frame_botones_superiores")
        ...
        self.bbt_nueva = tk.Button(self.frame_botones_superiores
                                   , name="bbt_nueva", text='NUEVA PARTIDA')
        self.bbt_nueva.grid(column=0, row=0, sticky="ew")
        self.bbt_salir = tk.Button(self.frame_botones_superiores
                                   , name="bbt_salir", text='TERMINAR')
        self.bbt_salir.grid(column=1, row=0, sticky="ew")

        self.bbt_ayuda = tk.Button(self.frame_botones_superiores
                                   , name="bbt_ayuda", text='AYUDA')
        self.bbt_ayuda.grid(column=0, row=1, sticky="ew")
        self.bbt_mostrar = tk.Button(self.frame_botones_superiores
                                     , name="bbt_mostrar", text='MOSTRAR')
        self.bbt_mostrar.grid(column=1, row=1, sticky="ew")

        # Frame para los botones de acción del turno
        self.frame_acciones_turno = tk.Frame(self.frame_botones
                                             , name="frame_acciones_turno")
        ...
        self.bbt_ok = tk.Button(self.frame_acciones_turno
                                , name="bbt_ok", text='OK - LISTO')
        self.bbt_ok.grid(column=0, row=0, sticky="ew")
        self.bbt_robar = tk.Button(self.frame_acciones_turno
                                   , name="bbt_robar", text='ROBAR')
        self.bbt_robar.grid(column=1, row=0, sticky="ew")
        self.bbt_atras = tk.Button(self.frame_acciones_turno
                                   , name="bbt_atras", text='ATRÁS/DESHACER')
        ...
        self.text_area = tk.Text(self.frame_botones
                                 , name="text_area"
                                 , height=5, width=80, font=small_font)
        ...
        self.frame_tablilla.configure(background="#097b1b"
                                      , height=200, width=200)
        self.frame_tablilla.grid(column=0, row=2, sticky="nsew")
        self.tablilla_botones = self._crear_botones_tablilla(
            self.frame_tablilla, "tablilla", dad.DraggableButton)
    ...
    def _crear_botones_tablilla(self, parent_frame, prefix, button_class):
        ...
        for i in range(3):
            ...
            for j in range(20):
                ...
                texto = ""
                btn_config = {}
                # Si es un botón normal (para la máquina), 
                # le damos un aspecto plano
                if button_class == tk.Button:
                    btn_config.update({'relief': tk.FLAT, 'borderwidth': 0})

                btn = button_class(parent_frame
                                   , name=btn_id, text=texto, **btn_config)                
    ...
    def _setup_fonts_and_colors(self):
        """Inicializa las fuentes y la configuración de 
    ...
    def _actualizar_fichas_en_bolsa(self, texto: str):
        """Actualiza el texto de la etiqueta que muestra el número de 
        fichas en la bolsa."""
        self.l_fichas_en_bolsa.config(text=texto)

    def _actualizar_tablilla(self, botones_tablilla, fichas):
        """Actualiza una tablilla (matriz de botones) con 
        una lista de fichas."""
        for i in range(3):
            for j in range(20):
                ...
                if fichas:
                    ...
                    try:
                        ...
                        # Los botones tk.Button (máquina) también deben
                        #  estar habilitados para poder pulsarlos.
                        else:
                            btn_config['state'] = 'normal'
    ...
    def _limpiar_mesa(self):
        """Limpia todos los botones de la mesa, dejándolos vacíos 
        y sin borde."""
    ...
    def _actualizar_mesa(self, fichas):
        """Actualiza la mesa de juego con una lista de fichas que están 
        en la mesa."""
        self._limpiar_mesa()
        ...
        # Ordenar las fichas por grupo para asegurar una
        ...
        for ficha in fichas:
            try:
                ...
                if (fila_actual >= len(self.mesa_botones) 
                    ...
                    print("Advertencia: No hay más espacio en la mesa"
